...
function flatten(data) {
  if (!(data instanceof Array)){
    ...
    let flattened = [];
    data.forEach( el => {
      el instanceof Array ? flattened.push(...flatten(el)) : flattened.push(el) 
    });
    return flattened;
  }
}
...
function fileFinder(directories, targetFile) {
  return Object.keys(directories).some( child => {
    if (directoryMatch.test(child)){
      return fileFinder(directories[child], targetFile);
    } else {
...
function pathFinder(directories, targetFile) {
  ...
  for ( let child in directories ){
    if (directoryMatch.test(child)) {
      let subPath = pathFinder(directories[child], targetFile);
      pathStr = subPath ? child + subPath : null;
    } else {
      pathStr = (child === targetFile) ? `/${child}` : null;
    }
